...
!!! warning "v0.1 — read this first"
    `django-yp-admin` is an **htmx-powered theme + helpers** for `django.contrib.admin`, **not** a full drop-in replacement. The current test suite (105 tests) exercises the theme, helpers, and a thin slice of the optional integrations. The compatibility claims below describe **design intent**, not regression-tested guarantees.
